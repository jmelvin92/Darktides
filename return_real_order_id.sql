-- Make finalize_order return the actual order_number from database
...
BEGIN
  ...
  IF p_customer_data IS NOT NULL THEN
    INSERT INTO orders (
      ...
      p_order_id,  -- Use exact frontend ID
    ...
    -- Get the actual order_number that was inserted
    SELECT order_number INTO v_actual_order_number 
    FROM orders WHERE id = v_order_uuid;
  ...
  -- Return the actual order_number from database
  RETURN QUERY SELECT true, COALESCE(v_actual_order_number, p_order_id)::TEXT;
END;
$$;