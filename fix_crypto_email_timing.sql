-- Fix crypto email timing issue
-- Drop ALL existing versions of finalize_order first
...
-- Now create the 6-parameter version
...
BEGIN
  ...
  BEGIN
    ...
    IF p_customer_data IS NOT NULL AND p_cart_items IS NOT NULL AND p_totals IS NOT NULL THEN
      ...
      -- Create the order - CRITICAL: Different status for crypto vs venmo
      INSERT INTO orders (
        order_number,
        customer_first_name,
        customer_last_name,
        ...
        p_cart_items,
        p_session_id,
        -- CRITICAL: Set status based on payment method
        CASE 
          WHEN p_payment_method = 'crypto' THEN 'pending' -- DO NOT trigger email yet!
          ELSE 'confirmed' -- Venmo orders trigger email immediately
        END,
        CASE 
...
-- Create the 5-parameter version that calls the 6 parameter one
CREATE OR REPLACE FUNCTION finalize_order(
  p_order_id TEXT,
  p_session_id TEXT,
  p_customer_data JSONB,
...
AS $$
BEGIN
  -- Default to venmo for backward compatibility
  RETURN QUERY 
...
-- Create function to confirm crypto orders after payment completes
