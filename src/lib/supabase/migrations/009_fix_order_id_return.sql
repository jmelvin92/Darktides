-- Fix the finalize_order function to return the order_number instead of UUID
-- This fixes the mismatch where Coinbase redirects with the frontend order ID
-- but the database was returning the UUID instead
...
BEGIN
  ...
  BEGIN
    ...
    -- CRITICAL FIX: Return the order_number (p_order_id) not the UUID!
    -- This ensures Coinbase redirect works with the correct order ID
...
-- Verify the fix
-- After running this, test a crypto order and the redirect should work with the correct order ID