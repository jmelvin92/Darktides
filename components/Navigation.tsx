...
interface NavProps {
  currentView: 'home' | 'store' | 'checkout' | 'order-complete' | 'calculator';
  onNavigate: (view: 'home' | 'store' | 'checkout' | 'order-complete' | 'calculator', sectionId?: string) => void;
  cartCount: number;
}
          ...
          {navLinks.map((link) => (
            <a 
              ...
              className={`text-[10px] font-mono uppercase tracking-[0.2em] hover:text-white transition-colors duration-300 relative group ${
                (link.id === 'home' && currentView === 'home') || (link.id === 'store' && currentView === 'store') || (link.id === 'calculator' && currentView === 'calculator') ? 'text-neon-blue' : 'text-gray-400'
              }`}
            >
              {link.label}
              <span className={`absolute -bottom-2 left-0 h-[1px] bg-neon-blue transition-all duration-300 ${
                 (link.id === 'home' && currentView === 'home') || (link.id === 'store' && currentView === 'store') || (link.id === 'calculator' && currentView === 'calculator') ? 'w-full' : 'w-0 group-hover:w-full'
              }`}></span>
            </a>
          ))}
