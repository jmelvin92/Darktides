  ...
  IF p_cart_items IS NOT NULL THEN
    ...
    LOOP
      ...
      INSERT INTO inventory_transactions (
        ...
        v_current_stock - (v_item.value->>'quantity')::INTEGER,
        p_order_id
      );
    END LOOP;
  END IF;
...
-- Remove any problematic triggers that were modified
...
RETURNS TRIGGER AS $$
BEGIN
  -- Simple notification that can be handled by the application
  RAISE NOTICE 'Order % confirmed (status: %)', NEW.order_number, NEW.status;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Create trigger that fires on confirmed orders
...
SELECT 'Reverted to original behavior: orders are created as confirmed' AS result;