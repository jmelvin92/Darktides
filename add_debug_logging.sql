-- Add debug logging to finalize_order function to catch where order ID changes
...
BEGIN
  -- CRITICAL DEBUG: Log what order ID we received
  RAISE NOTICE '🔥 DATABASE FUNCTION RECEIVED ORDER ID: %', p_order_id;
  ...
  IF p_customer_data IS NOT NULL AND p_cart_items IS NOT NULL AND p_totals IS NOT NULL THEN
    ...
    RAISE NOTICE '🔥 DATABASE INSERTING ORDER_NUMBER: %', p_order_id;
    ...
    -- CRITICAL DEBUG: Log what actually got inserted
    DECLARE
      v_actual_order_number TEXT;
    BEGIN
      SELECT order_number INTO v_actual_order_number 
      FROM orders 
      WHERE id = v_order_uuid;
      
      RAISE NOTICE '🔥 DATABASE INSERTED ORDER_NUMBER: %', v_actual_order_number;
    END;
